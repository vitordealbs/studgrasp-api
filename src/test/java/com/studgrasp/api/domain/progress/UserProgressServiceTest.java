...
class UserProgressServiceTest {
    ...
    void shouldCreateNewProgressWhenItDoesNotExist() {
        ...
        UserProgress saved = UserProgress.builder()
                .id(progressId).user(user).roadmapNode(node)
                .status("IN_PROGRESS").updatedAt(LocalDateTime.now()).build();
        ...
        UserProgressResponseDTO response = userProgressService.updateProgress(user, request);
    ...
    void shouldUpdateExistingProgressWhenItAlreadyExists() {
        ...
        UserProgress existing = UserProgress.builder()
                .id(UUID.randomUUID()).user(user).roadmapNode(node).status("IN_PROGRESS").build();

        when(roadmapNodeRepository.findById(nodeId)).thenReturn(Optional.of(node));
        when(userProgressRepository.findByUserIdAndRoadmapNodeId(userId, nodeId))
    ...
    void shouldThrowResourceNotFoundExceptionWhenRoadmapNodeDoesNotExist() {
        ...
        assertThrows(ResourceNotFoundException.class,
                () -> userProgressService.updateProgress(user, request));
    }
}
